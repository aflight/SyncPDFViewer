...
- (id)initWithDelegate:(id)delegate userData:(long)userData
{
	if(self = [super init])
	{
		...
		theRunLoopModes = [[NSArray arrayWithObject:NSRunLoopCommonModes] retain];
	}
	return self;
}
...
//	if (theSocket6)	setsockopt(CFSocketGetNative(theSocket6), SOL_SOCKET, SO_REUSEADDR, &reuseOn, sizeof(reuseOn));
	if (theSocket) {
		setsockopt(CFSocketGetNative(theSocket), SOL_SOCKET, SO_REUSEADDR, &reuseOn, sizeof(reuseOn));
		int set = 1; 
		setsockopt(CFSocketGetNative(theSocket), SOL_SOCKET, SO_NOSIGPIPE, (void *)&set, sizeof(int));	
	}
	if (theSocket6)	{
		setsockopt(CFSocketGetNative(theSocket6), SOL_SOCKET, SO_REUSEADDR, &reuseOn, sizeof(reuseOn));
		int set = 1; 
		setsockopt(CFSocketGetNative(theSocket6), SOL_SOCKET, SO_NOSIGPIPE, (void *)&set, sizeof(int));
	}
	//TODO ZWH MODIFIED ENDED
				...
				error:(NSError **)errPtr
{
	if(theDelegate == NULL)
	{
		...
		[NSException raise:AsyncSocketException format:@"ERROR:%@", message];
	}
